// colour.js — NEVES ONE · Five Poses
// ---------------------------------------------------------------------------
// Hex handling for the product colourway override.
//
// Image engines read a hex value far more reliably when it arrives alongside a
// plain-language description of the same colour. "#1B2A4A" on its own gets
// approximated; "a deep rich indigo blue, hex #1B2A4A" lands much closer.
// Everything here exists to produce that second half.
// ---------------------------------------------------------------------------
...
const HUES = [
  [10, "red"], [22, "red-orange"], [38, "orange"], [50, "amber"],
  [66, "yellow"], [82, "yellow-green"], [100, "lime green"], [145, "green"],
  [168, "green-teal"], [186, "teal"], [200, "cyan"], [225, "blue"],
  [250, "indigo blue"], [268, "violet"], [290, "purple"], [318, "magenta"],
  [340, "pink"], [360, "red"],
];

function hueName(h) {
  for (const [ceiling, name] of HUES) if (h <= ceiling) return name;
  return "red";
}

// Neutrals read better by name than as "a zero-saturation grey".
...
// A short plain-language name for a hex value, e.g. "deep rich indigo blue".
export function describeHex(hex) {
  const rgb = hexToRgb(hex);
  if (!rgb) return "";
  const { h, s, l } = rgbToHsl(rgb);
  if (s < 0.08) return neutralName(l);
  return [lightnessWord(l), saturationWord(s), hueName(h)].filter(Boolean).join(" ");
}

// The full phrase used inside the prompt: name first, hex second.
...
// Black or white ink for a swatch of this colour, for the UI.
