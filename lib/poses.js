...
// Colourway override. PRODUCT_PRESERVE says "Same colour", which is correct for
// every normal run, so this block has to state plainly that it outranks that
// line, otherwise the two instructions average out into a half-shifted hue.
export function buildColourOverride({ phrase, scope = 'the whole garment' }) {
  if (!phrase) return '';
  return `GARMENT COLOURWAY OVERRIDE (THIS SUPERSEDES THE "Same colour" LINE IN GARMENT CONSTRUCTION ABOVE):
- The garment's colour is being deliberately changed. Render ${scope} in ${phrase}, as that colour would photograph under neutral studio light.
- This is a dye change, not a repaint. The fabric keeps its exact weave, knit, pile, sheen, weight and drape. Only the hue changes.
- The hex value describes the fabric on the flat, evenly lit areas. Do not flood the garment with one uniform block of that hex. Shading stays physically correct: the colour reads darker inside folds, creases and shadow, and lighter on the faces turned toward the key light.
- Pattern geometry does not change at all. Motif shape, motif count, repeat pitch, stripe widths, stripe count and check grid stay exactly as referenced. Only the colours sitting within that unchanged geometry shift to work against the new base.
- Where the reference garment has contrast trims, collars, cuffs, plackets, tipping or hems in a different colour from the body, those keep their relationship to the new base. If a trim was lighter than the body in the reference, it stays lighter; if darker, it stays darker.
- The brand emblem, badge, crest or embroidery keeps its own original colours exactly as referenced. Never recolour the mark to match the garment, and never let it disappear into the new base.
- Buttons, zips, eyelets, drawcords and hardware keep their referenced colour and finish, unless they were an exact match to the body fabric in the reference, in which case they follow it.
- Skin tone, hair, footwear, lower garment, background and lighting are completely unaffected by this override.`;
}

// Pairing rules for the optional footwear and lower-garment slots. Only emitted
// when something is actually supplied, so a plain top-only run reads exactly as
// it did before.
export function buildPairingBlock({ hasBottoms = false, hasFootwear = false } = {}) {
  if (!hasBottoms && !hasFootwear) return '';
  const lines = ['STYLING AND PAIRING:'];
  lines.push(
    '- The main garment is the hero product. The paired items below are styling: reproduce them accurately, but they must never draw attention away from the hero garment or crowd it.'
  );
  if (hasBottoms) {
    lines.push(
      '- LOWER GARMENT: the model wears the exact lower garment shown in the supplied reference. Same cut, same leg shape, same rise, same length, same wash or colour, same fabric behaviour, same pocket and seam detail. Do not substitute a generic pair, do not restyle it, and do not change its colour.',
      '- The lower garment sits correctly on the body: waistband at the right height, correct break at the ankle, natural fabric weight and fall. Where the hero garment is tucked or untucked in the reference, keep that relationship consistent across every frame of the set.'
    );
  }
  if (hasFootwear) {
    lines.push(
      '- FOOTWEAR: the model wears the exact pair shown in the supplied reference. Same silhouette, same upper, same sole unit, same colour blocking, same branding placement. Both shoes match each other, are correctly scaled to the body, and sit flat on the floor plane.',
      '- Do not swap the footwear between frames, and do not invent a different pair when the feet are partly out of shot.'
    );
  }
  lines.push(
    '- Paired items keep the same identity in every frame of the set. They are as fixed as the hero garment.'
  );
  return lines.join('\n');
}
...
const ROLE_TEXT = {
  ...
  bottoms:
    'the LOWER GARMENT to be worn with the hero garment — trousers, jeans, shorts or a skirt. Reproduce this exact item: same cut, leg shape, rise, length, wash or colour, fabric behaviour, pocket and seam detail. This is styling, not the hero product, but it is still a real product and must not be substituted or reinvented.',
  bottomsSheet:
    'a CONTACT SHEET of additional photographs of THE SAME SINGLE LOWER GARMENT, arranged in a grid on white. Every panel shows that one identical item from a different angle or in detail. This is ONE item, not several: read every panel and reproduce that exact lower garment.',
  anchor:
...
- Do not re-interpret the pattern between frames. The motif count and repeat pitch in the anchor are the motif count and repeat pitch here.`
  ...
  return assemble([
    ...
    buildDefectRules(),
    PRODUCT_PRESERVE,
    colour ? buildColourOverride(colour) : '',
    pairing ? buildPairingBlock(pairing) : '',
    `MODEL:\n${modelDescriptor}`,
    MODEL_RULE,
    isChild ? CHILD_RULE : '',
    SKIN_REALISM,
