...
${STUDIO}`;
  ...
  return applyStrictFidelity(base, {
    strict,
    roleLines,
    permitted: permitted || ["the model's pose and the camera framing, as described under POSE / FRAMING"],
    productDesc,
    withPerson: true,
  });
}
...
export function buildReframePrompt({ posePrompt, gender, faceDesc, productDesc, notes, strict = false, roleLines = "", permitted = null }) {
...
${STUDIO}`;
  ...
  return applyStrictFidelity(base, {
    strict,
    roleLines,
    permitted: permitted || ["the camera framing and the body pose"],
    productDesc,
    withPerson: true,
  });
}
...
export function buildRefinePrompt(instruction, { strict = false, roleLines = "", productDesc = "" } = {}) {
...
export function buildStrictFidelityBlock({ roleLines, permitted, productDesc, withPerson = true }) {
...
export function applyStrictFidelity(basePrompt, { strict, roleLines, permitted, productDesc, withPerson }) {
  if (!strict) return basePrompt;
  return `${basePrompt}\n${buildStrictFidelityBlock({ roleLines, permitted, productDesc, withPerson })}`;
}
...
FRAMING — ${slotFraming}, on a seamless pure-white background, hex #FFFFFF, with soft even studio lighting, sharp focus, true-to-life colour and material rendering and high detail. No props, no text overlays, no added graphics or logos.`;

  return applyStrictFidelity(base, {
    strict,
    roleLines,
    permitted: ["the camera angle and framing of the garment, as described under FRAMING"],
    productDesc,
    withPerson: false,
  });
}
